...
def display_inventory(filter_text=""):
    try:
        ...
        if not conn or not c:
            messagebox.showerror("Database Error", "Database connection is not initialized")
            return
            
        query = "SELECT id, item_name, quantity, price, updated_by FROM inventory"
        params = ()
        if filter_text:
...
action_frame.place(x=20, y=520, width=380, height=200)  # Adjusted y position to accommodate Updated By field
...
# Configure style for context menu
...
inventory_tree.bind('<Button-3>', show_context_menu)  # Right-click
...
def treeview_sort_column(tree, col, reverse):
    ...
    # Convert values based on column type for proper sorting
    if col in ["Quantity"]:
...
for col in columns:
    inventory_tree.heading(col, command=lambda c=col: treeview_sort_column(inventory_tree, c, False))
